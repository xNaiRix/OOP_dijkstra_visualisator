...
bool DijkstraSolver::step() {
    ...
    qDebug() << "=== Step: is relacsation ==" << relacsationPhase;
...
bool DijkstraSolver::nextYellow(){
    ...
    // 5. Правило 2.2: оранжевая вершина -> жёлтая (инцидентная выбранному жёлтому ребру)
        ...
        for (auto vert: {u,v}){
            if (m_vertexState.value(vert, VS_WHITE) == VS_ORANGE){
                setVertexState(vert, VS_YELLOW);
                m_currentVertex = vert;
                qDebug() << "  Rule 2.2: vertex" << vert->getId() << "set YELLOW";
            }
        }
    }
